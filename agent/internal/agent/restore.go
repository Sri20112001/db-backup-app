...
func RestoreLocal(archivePath, destDir string, progress ProgressFunc) (int64, error) {
	...
	// Sniff gzip magic (1f 8b) rather than trusting the file extension:
	// decrypted temp files (.dec) and renamed archives may not end in .gz.
	magic := make([]byte, 2)
	if _, err := io.ReadFull(f, magic); err != nil {
		return 0, fmt.Errorf("read archive header: %w", err)
	}
	 rest := io.MultiReader(bytes.NewReader(magic), f)
